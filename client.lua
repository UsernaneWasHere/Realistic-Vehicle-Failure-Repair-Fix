...
--
--  Remastered by Usernane_Was_Here#8008
--
...
local CanRepair = true -- Determines wether or not the car is repairable
local repairs = 0 -- Amount of times the car has been repaired
local Oil = 0 -- Ammount of times the oil cap was tightened
local Spark = 0 -- Amount of times the spark plug was tightened
local Filter = 0
local Problem = "Nothing" -- Text that is outputed when a problem is fixed

RegisterCommand("repair", function()
	local ProblemNumber = math.random(1, 3) -- Generates a random number between 1 and 3
	local ped = PlayerPedId()
	local vehicle = GetVehiclePedIsIn(ped, false)
	if Oil >= 2 or Spark >= 2 or repairs >= 2 then -- If the Oil cap, Spark plug, or total repairs equals to 2 then it will set the CanRepair to false
		CanRepair = false
	end

	if ProblemNumber == 1 then -- If the Problem Number is equal to 1 it will set the output text
		Problem = "You tightened your spark plug and got your car running!"
		Spark = Spark + 1
	end


	if ProblemNumber == 2 then -- If the Problem Number is equal to 1 it will set the output text
		Problem = "You put the oil plug back in, now get to a mechanic!"
		Oil = Oil + 1
	end

	if ProblemNumber == 3 then -- If the Problem Number is equal to 1 it will set the output text
		Problem = "You put the air filter back on, now get to a mechanic!"
		Filter = Filter + 1
	end
	...
	if vehicle ~= 0 and GetPedInVehicleSeat(vehicle, -1) == ped and CanRepair == true then
		...
		healthEngineCurrent = 650.0
		healthEngineNew = 1000.0
		healthEngineDelta = 0.0
		healthEngineDeltaScaled = 0.0
		SetVehicleUndriveable(vehicle, false)
		TriggerEvent('chatMessage', "Repair", {200,0,0}, Problem)
		repairs = repairs + 1
	else
			TriggerEvent('chatMessage', "Repair", {200,0,0}, "You were unsucessful in repairing your vehicle")
		end
end)
